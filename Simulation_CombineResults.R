...
###BIAS AND ERROR Superimposed into once chart (NEW)
  ...
  error_1_plot<-ggplot() + ylim(0,.1) + labs(x = TeX("Kendall's $\\tau$"), y=TeX("$SE(\\hat{\\beta_{\\mu_1}})$")) +
  ...
  error_2_plot<-ggplot() + ylim(0,.1) + labs(x = TeX("Kendall's $\\tau$"), y=TeX("$SE(\\hat{\\beta_{\\mu_2}})$ | $SE(\\hat{\\beta_{\\mu_t}})$")) +
