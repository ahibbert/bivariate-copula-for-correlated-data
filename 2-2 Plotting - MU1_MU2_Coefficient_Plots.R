...
prepare_mu2_plot_data <- function(dist_name, models_to_plot = NULL) {
  ...
  if(bt_mode==TRUE) {
    if(dist_name %in% c("GA")) {
      true_mu2 <- log(
        exp((as.numeric(true_params_matrix$mu2[dist_rows])*as.numeric(true_params_matrix$a[dist_rows])))/
        exp((as.numeric(true_params_matrix$mu1[dist_rows])*as.numeric(true_params_matrix$a[dist_rows])))
      )
    } else if (dist_name == "PO") {
      true_mu2 <- log(
        (as.numeric(true_params_matrix$mu2[dist_rows])*as.numeric(true_params_matrix$b[dist_rows])*as.numeric(true_params_matrix$c[dist_rows]))/
        (as.numeric(true_params_matrix$mu1[dist_rows])*as.numeric(true_params_matrix$b[dist_rows])*as.numeric(true_params_matrix$c[dist_rows]))
      )
    } else if(dist_name == "LO") {
    ...
    if(dist_name %in% c("GA")) {
      true_mu2 <- ((as.numeric(true_params_matrix$mu2[dist_rows])*as.numeric(true_params_matrix$a[dist_rows])))
    } else if (dist_name == "PO") {
      true_mu2 <- ((as.numeric(true_params_matrix$mu2[dist_rows])*as.numeric(true_params_matrix$b[dist_rows])*as.numeric(true_params_matrix$c[dist_rows])))
    } else if(dist_name == "LO") {
...
if(length(bias_plots_skew) > 0 && length(se_plots_skew) > 0) {
  ...
  if(!is.null(shared_legend_skew)) {
    ...
    ggsave(chart_filename_skew, plot = combined_plot_skew, width = 14, height = 11, dpi = 600)
...
if(length(bias_plots_mu2_skew) > 0 && length(se_plots_mu2_skew) > 0) {
  ...
  if(!is.null(shared_legend_mu2_skew)) {
    ...
    ggsave(chart_filename_mu2_skew, plot = combined_plot_mu2_skew, width = 14, height = 11, dpi = 600)
