...
bic_matrix <- matrix(NA, nrow = n_param_sets, ncol = n_models,
                     dimnames = list(row_names, model_names))
aic_matrix <- matrix(NA, nrow = n_param_sets, ncol = n_models,
                     dimnames = list(row_names, model_names))
gaic_matrix <- matrix(NA, nrow = n_param_sets, ncol = n_models,
                     dimnames = list(row_names, model_names))
...
for (i in seq_along(all_results)) {
  ...
  tryCatch({
    ...
    if ("logliks" %in% names(result$score_items) && !is.null(result$df_values$logliks)) {
      ...
      aic_matrix[i, ] <- -2 * log_lik + 2 * df
      gaic_matrix[i, ] <- -2 * log_lik + 4 * df
    }
