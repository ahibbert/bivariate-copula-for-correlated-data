...
fitBivModels_Bt_withCov <-function(dataset,dist,include="ALL",a,b,c,mu1,mu2,calc_actuals=TRUE,bt_mode=FALSE) {
  ...
  if(include=="ALL" || include=="non-GJRM" ) {
    ...
    sigmas=rbind(
      ...
      , rep(model_gee$phi,2)
      , if(dist=="LO") { rep(1,2) } else {rep((model_re_nosig$sigma.coefficients),2)}
      , if(dist=="LO") { rep(1,2) } else {model_re_np$sigma.coefficients} #Note this isn't used anywhere as this model isn't included for evaluation
...
sim_model <- function(model,dist,n,coefficients,sigmas,correlations,bt_mode=FALSE) {
  ...
  if (model == "glm") {
    ...
    if(dist=="NO") {
      ...
      t2=rBI(n, mu=logit_inv(lp_2+re_val))#*exp(re_val)
    } else if (dist=="PO") {
    ...
    if(is.na(cop_vine)) {
      ...
      t2=rep(NA,n)
        ...
        if(dist=="NO") {
          ...
          t1=qBI(simCop[,1], mu=logit_inv(lp_1))
          t2=qBI(simCop[,2], mu=logit_inv(lp_2))
        } else if (dist=="PO") {
