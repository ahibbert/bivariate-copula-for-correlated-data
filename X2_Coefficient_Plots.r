...
prepare_x2_plot_data <- function(dist_name, models_to_plot = NULL) {
  ...
  if(is.null(models_to_plot)) {
    non_missing_counts <- colSums(!is.na(x2_coef_matrix))
  ...
  est_x2_matrix <- x2_coef_matrix[dist_rows, models_to_plot, drop = FALSE]
...
prepare_x2_se_plot_data <- function(dist_name, models_to_plot = NULL) {
  ...
  # If no models specified, use all available models with non-missing data
  if(is.null(models_to_plot)) {
    non_missing_counts <- colSums(!is.na(x2_se_matrix))
    models_to_plot <- names(non_missing_counts[non_missing_counts > 0])
  }
  ...
  est_se_matrix <- x2_se_matrix[dist_rows, models_to_plot, drop = FALSE]
...
if(length(bias_plots_x2) > 0 && length(se_plots_x2) > 0) {
  ...
  if(is.null(legend_dist_x2)) {
    ...
    shared_legend_x2 <- validate_legend(NULL, "X2 Plots", force_manual = TRUE)
  } else {
    ...
    tryCatch({
      ...
      shared_legend_x2 <- validate_legend(shared_legend_x2, "X2 Plots", force_manual = FALSE)
...
for(model in models_to_plot) {
  if(model %in% colnames(x2_coef_matrix)) {
    non_missing <- sum(!is.na(x2_coef_matrix[, model]))
...
for(model in models_to_plot) {
  if(model %in% colnames(x2_se_matrix)) {
    non_missing <- sum(!is.na(x2_se_matrix[, model]))
    model_label <- rename_model(model)
