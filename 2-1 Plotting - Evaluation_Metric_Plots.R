...
required_objects <- c("vs2_matrix", "vs2_wt_matrix", "loglik_matrix", "bic_matrix", "true_params_matrix", 
                      ...
                      "get_legend", "validate_legend", "DEBUG_LEGENDS")
...
# Clean GAIC matrix if it exists
if(exists("gaic_matrix") && !is.null(gaic_matrix)) {
  gaic_matrix_clean <- remove_outliers_eval(gaic_matrix, "gaic_matrix", verbose = TRUE)
  cat("GAIC matrix cleaned.\n")
} else {
  cat("GAIC matrix not found, skipping outlier removal\n")
  gaic_matrix_clean <- NULL
}
...
prepare_eval_data <- function(dist_name, metric_matrix, metric_name, models_to_use = NULL, use_cleaned_data = TRUE) {
  ...
  if(use_cleaned_data) {
    ...
    } else if(metric_name == "GAIC" && exists("gaic_matrix_clean") && !is.null(gaic_matrix_clean)) {
      metric_matrix <- gaic_matrix_clean
      cat("Using cleaned GAIC data for", dist_name, "\n")
    }
  }
...
create_eval_metric_plot <- function(metric_matrix, metric_name, main_title, y_label, 
  ...
  # Add safety check for NULL legend
  if(!is.null(shared_legend)) {
    ...
    cat("Warning: Could not create legend for", metric_name, ", creating plot without legend\n")
    combined_plot <- arrangeGrob(
      plots_grid, 
      ncol = 1,
      top = textGrob(main_title, gp = gpar(fontsize = 16, fontface = "bold"))
...
# 5. AIC Plot
cat("\n--- AIC Analysis ---\n")
# Exclude GEE and GAMM models for AIC plots (same as BIC)
aic_models_to_use <- setdiff(eval_models_to_plot, c("gee", "gamm"))
cat("Models for AIC plots (excluding GEE and GAMM):", paste(aic_models_to_use, collapse = ", "), "\n")
...
if(exists("aic_matrix") && !is.null(aic_matrix)) {
  ...
  if(exists("aic_matrix_clean") && !is.null(aic_matrix_clean)) {
    aic_plot <- create_eval_metric_plot(
      ...
      y_label = "AIC",
      models_to_use = aic_models_to_use,
      y_limits = NULL,  # Auto-scale
      lower_is_better = TRUE
    )
  } else {
  ...
  cat("AIC matrix not found, skipping AIC plot.\n")
  aic_plot <- NULL
}

# 6. GAIC Plot
cat("\n--- GAIC Analysis ---\n")
# Exclude GEE and GAMM models for GAIC plots (same as BIC)
gaic_models_to_use <- setdiff(eval_models_to_plot, c("gee", "gamm"))
cat("Models for GAIC plots (excluding GEE and GAMM):", paste(gaic_models_to_use, collapse = ", "), "\n")

if(exists("gaic_matrix") && !is.null(gaic_matrix)) {
  # GAIC matrix exists, use cleaned version if available
  if(exists("gaic_matrix_clean") && !is.null(gaic_matrix_clean)) {
    gaic_plot <- create_eval_metric_plot(
      metric_matrix = gaic_matrix_clean,
      metric_name = "GAIC",
      main_title = "Generalized Akaike Information Criterion (GAIC) by Distribution",
      y_label = "GAIC",
      models_to_use = gaic_models_to_use,
      y_limits = NULL,  # Auto-scale
      lower_is_better = TRUE
    )
  } else {
    cat("Warning: Cleaned GAIC matrix not available, using original data\n")
    gaic_plot <- create_eval_metric_plot(
      metric_matrix = gaic_matrix,
      metric_name = "GAIC",
      main_title = "Generalized Akaike Information Criterion (GAIC) by Distribution",
      y_label = "GAIC",
      models_to_use = gaic_models_to_use,
  ...
  if(!is.null(gaic_plot)) {
    # Save the plot
    chart_filename_gaic <- paste0("Charts/GAIC_by_Distribution_", Sys.Date(), ".png")
    # Save the plot with adjusted dimensions for single row layout
    ggsave(chart_filename_gaic, plot = gaic_plot, width = 20, height = 6, dpi = 600)
    cat("GAIC plot saved to:", chart_filename_gaic, "\n")
  } else {
    cat("Could not create GAIC plot\n")
  }
} else {
  cat("GAIC matrix not found, skipping GAIC plot.\n")
  gaic_plot <- NULL
}
...
if(exists("gaic_plot") && !is.null(gaic_plot)) cat("✓ GAIC\n") else cat("✗ GAIC\n")
